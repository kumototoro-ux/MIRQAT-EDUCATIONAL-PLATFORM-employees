...
/** يبني تسمية أسبوع لا لبس فيها للرسوم البيانية: الاسم + تاريخ مختصر (لأن أسماء الأسابيع قد تتكرر بين الترمات) */
export function formatWeekLabel(w) {
  if (!w) return '';
  const d = w.week_start_date ? new Date(w.week_start_date) : null;
  const shortDate = d ? `${d.getMonth() + 1}/${d.getDate()}` : '';
  return shortDate ? `${w.week} (${shortDate})` : w.week;
}
